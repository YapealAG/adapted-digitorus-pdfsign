...
const (
	subFilterETSICAdES   = "/ETSI.CAdES.detached"
	subFilterETISRFC3161 = "/ETSI.RFC3161"
)
...
func ReaderWithRootPool(
	...
	for _, x := range rdr.Xref() {
		...
		for i := 0; i < v.Key("ByteRange").Len(); i++ {
			...
			contentInRange = append(contentInRange, content...)
		}
...
func processCAdES(
	...
	for _, s := range p7.Signers {
		...
		for _, attr := range s.UnauthenticatedAttributes {
			...
			if attr.Type.Equal(asn1.ObjectIdentifier{1, 2, 840, 113549, 1, 9, 16, 2, 14}) {
				...
				if err != nil {
					err = fmt.Errorf("Failed to parse timestamp %v", err)
					return
				} else {
					...
					if !bytes.Equal(h.Sum(nil), signer.TimeStamp.HashedMessage) {
						err = fmt.Errorf("Hash in timestamp is different from pkcs7 %v", err)
					}
	...
	for _, o := range revInfo.OCSP {
		...
		if err != nil {
			err = fmt.Errorf("Failed to parse or verify OCSP response %v", err)
